...
- (void)setupRemoteVideoView {
    RCRTCRemoteVideoView *view = [[RCRTCRemoteVideoView alloc] initWithFrame:CGRectMake(0,
        (UIScreen.mainScreen.bounds.size.height - UIScreen.mainScreen.bounds.size.width) / 2,
...
- (void)subscribeLiveStream {
    @WeakObj(self);
    [[RCRTCEngine sharedInstance] subscribeLiveStream:self.liveUrl streamType:RCRTCAVStreamTypeAudioVideo completion:^(
        RCRTCCode desc,
        RCRTCInputStream *_Nullable inputStream) {
        @StrongObj(self);
