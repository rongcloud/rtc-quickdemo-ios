...
- (void)publishLocalLiveAVStream {
    ...
    // 发布本地流到房间
    [self.room.localUser publishDefaultLiveStreams:^(BOOL isSuccess,
        RCRTCCode desc,
        ...
        if (desc == RCRTCCodeSuccess) {
            ...
            [liveInfo setMixStreamConfig:[self setOutputConfig:RCRTCMixLayoutModeCustom] completion:^(BOOL isSuccess,
                RCRTCCode code) {

            }];
            NSLog(@"本地发布成功: liveUrl = %@", liveInfo.liveUrl);
            alert.message = liveInfo.liveUrl;
            UIAlertAction *okAtion =
                [UIAlertAction actionWithTitle:@"复制 liveUrl 到剪切板" style:UIAlertActionStyleDefault handler:^(
