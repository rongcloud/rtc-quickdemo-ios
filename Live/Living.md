...
`/Live/LivingTool`：里面包括直播的合流布局配置类`/Live/LiveMixStreamTool`，直播本地发布订阅的音视频流的布局类`/Live/LiveVideoLayoutTool`，预览图创建标识 Id
类`/Live/LiveStreamVideo`。
...
4. 远端用户发布音视频流后，创建并设置远端视频流的渲染图并添加在本地视图上，然后可以通过加入房间后返回的 `RCRTCRoom` 中的 localUser 中的订阅方法订阅多路远端指定音视频流。同一个流只能填写在 avStreams 或
   tinyStreams 中的一个数组中。
